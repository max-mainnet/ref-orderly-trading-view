...
import { NearIcon, NearWalletIcon, FirstPage, PrePage, NextPage, LastPage } from '../Common/Icons';
...
function AssetButton({
  ...
  return (
    <button
      className={`${className} px-2 py-1.5 text-white ${forbid ? 'cursor-not-allowed' : ''} rounded-lg ${
  ...
  return (
    ...
    >
      <FlexRow className='col-span-2 '>
        <img src={props.tokenMeta.icon} className='rounded-full mr-2 w-7 h-7 border border-green-400' alt='' />
        <div className='flex flex-col '>
          <div className='text-white font-bold'>{props.tokenMeta.symbol}</div>

          <div className='text-primary text-xs'>{getAccountName(props.tokenMeta.id)}</div>
        </div>
      </FlexRow>

      <FlexRow className='justify-self-end relative right-4'>{digitWrapper(props.near, 3)}</FlexRow>

      <FlexRow className='col-span-2 justify-self-end relative right-12'>{digitWrapper(props['in-order'], 3)}</FlexRow>

      <FlexRow className='relative justify-self-end right-8'>{digitWrapper(props.available, 3)}</FlexRow>
...
function RecordLine(
  ...
  return (
    <div className='grid grid-cols-6 justify-items-end py-3 text-gray2 pl-5 '>
      ...
      <FlexRow className='justify-self-end relative  right-24'>{digitWrapper(props.amount.toString(), 3)}</FlexRow>

      <FlexRow className='col-span-1 justify-self-end relative right-14'>{window.selectorAccountId}</FlexRow>
      ...
      <FlexRow className='relative justify-self-end whitespace-nowrap '>{formatTimeDate(props.updated_time)}</FlexRow>

      <FlexRow className={`justify-center justify-self-center ${props.side === 'WITHDRAW' ? 'text-withdrawPurple3' : 'text-baseGreen'}`}>
...
export function AssetModal(props: Modal.Props) {
  ...
  const [sortBy, setSortBy] = useState<'near' | 'in-order' | 'available'>('available');
  ...
  useEffect(() => {
    if (records === undefined || (DEFAULT_PAGE_SIZE * curPage > records.length && records.length < total)) {
      if (!window.selectorAccountId) return;
  ...
  return (
    <Modal {...props}>
      <div className=' rounded-2xl overflow-hidden lg:w-p869 xs:w-95vw gradientBorderWrapperNoShadow bg-boxBorder text-sm text-primary border '>
        <div className=' flex flex-col '>
          <div className='flex bg-allOrderHeader pt-4 px-5 pb-4  items-center  justify-between'>
            <div className='text-white  text-base font-bold'>
              <span
                className={tag !== 'asset' ? 'text-primary cursor-pointer relative' : 'cursor-pointer relative'}
                onClick={() => {
                  setTag('asset');
                }}
              >
              ...
              <span
                className={tag !== 'records' ? 'text-primary ml-20 cursor-pointer relative' : 'ml-20 cursor-pointer relative'}
                onClick={() => {
                  setTag('records');
                }}
              >
          ...
          {tag === 'asset' && (
            <div className='grid grid-cols-8 px-5  py-3 h-14'>
              <div className='col-span-2 text-center  inline-flex items-center justify-center justify-self-start'>Tokens</div>
            ...
            `}
              ...
              >
                ...
                <MdArrowDropDown
                  ...
                  className={`${orderBy === 'asc' && sortBy === 'near' ? 'transform rotate-180' : ''} `}
                ></MdArrowDropDown>
              ...
              <div
                className={`col-span-2 cursor-pointer flex justify-self-center items-center ${sortBy === 'in-order' ? 'white' : '#7E8A93'}`}
                onClick={() => {
                  setSortBy('in-order');
                  OrderBy(orderBy === 'desc' ? 'asc' : 'desc');
                }}
              >
                ...
                <MdArrowDropDown
                  ...
                  className={`${orderBy === 'asc' && sortBy === 'in-order' ? 'transform rotate-180' : ''} `}
                ></MdArrowDropDown>
              </div>

              <div
                className={`${sortBy === 'available' ? 'white' : '#7E8A93'} cursor-pointer flex items-center justify-self-center justify-center`}
                onClick={() => {
                  setSortBy('available');
                  OrderBy(orderBy === 'desc' ? 'asc' : 'desc');
                }}
              >
                ...
                <MdArrowDropDown
                  ...
                  className={`${orderBy === 'asc' && sortBy === 'available' ? 'transform rotate-180' : ''} `}
                ></MdArrowDropDown>
              </div>
          ...
          {tag === 'records' && (
            <div className='grid border-b border-gray1 grid-cols-6 pl-5 pr-0 justify-items-center text-primary py-3 h-14'>
              <div className='col-span-1 text-center  inline-flex items-center justify-center justify-self-start'>Token</div>
              ...
              <div className={`col-span-1  flex justify-items-center relative right-10  items-center `}>
          ...
          {tokenInfo &&
            (tag === 'asset' ? (
              ...
              records &&
              records?.slice((curPage - 1) * DEFAULT_PAGE_SIZE, curPage * DEFAULT_PAGE_SIZE).map((r) => {
